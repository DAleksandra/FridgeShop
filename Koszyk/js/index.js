...
Number.prototype.to_z = function () {
  return parseFloat( this ).toFixed(2) + "zł";
};
String.prototype.stripz = function () {
  return this.split("zł")[0];
};
...
var app = {
  ...
  updateProductSubtotal: function (context, quantity) {
    ...
    subtotalCtr.html( subtotalPrice.to_z() );
  ...
  updateTotals: function () {
    ...
    for (var i = 0; i < products.length; i += 1) {
      subtotal += parseFloat( $(products[i]).find(".product-total-price").html().stripz() );
    }

    shipping = (subtotal > 0 && subtotal < (100 / 1.06)) ? app.shipping : 20;

    $("#subtotalCtr").find(".cart-totals-value").html( subtotal.to_z() );
    $("#taxesCtr").find(".cart-totals-value").html( (subtotal * 0.12).to_z() );
    $("#totalCtr").find(".cart-totals-value").html( (subtotal + shipping).to_z() );
    $("#shippingCtr").find(".cart-totals-value").html( shipping.to_z() );
   
    fillPrice(subtotal + shipping);
